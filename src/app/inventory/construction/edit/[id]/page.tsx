...
import { getConstructionServiceById, updateConstructionService } from "@/lib/actions/construction-actions";

export default function EditConstructionServicePage() {
  ...
  useEffect(() => {
    if (id) {
      const fetchService = async () => {
        setIsLoading(true);
        const result = await getConstructionServiceById(id);
        if (result.success && result.service) {
          ...
          setFormData({
            ...
            accomplishedTitle: s.accomplishedTitle || "What We've Accomplished Together",
            accomplishedDescription: s.accomplishedDescription || "",
            accomplishedQuote: s.accomplishedQuote || "",
            overviewTitle: s.overviewTitle || "Services Overview",
            overviewDescription: s.overviewDescription || "",
            features: s.features || [],
            highlightFeatures: s.highlightFeatures || [],
          ...
          router.push("/inventory/construction");
        }
        setIsLoading(false);
      };
      fetchService();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await updateConstructionService(id, formData);

      if (result.success) {
        toast.success("Construction service updated successfully!");
        router.push("/inventory/construction");
      } else {
  ...
  if (isLoading) {
    return (
      <AppLayout title="Loading..." subtitle="Fetching service details">
  ...
  return (
    <AppLayout
      title="Edit Construction Service"
      subtitle="Modify the details of an existing construction service."
      actions={
