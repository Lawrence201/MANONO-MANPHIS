...
import { getBillboard, updateBillboard } from "@/lib/actions/billboard-actions";
import { toast } from "sonner";

export default function EditBillboardPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    const fetchBillboard = async () => {
      ...
      if (result.success && result.data) {
        // Normalize DB values to match UI labels
        const data = {
          ...result.data,
          screenType: result.data.screenType === "led" ? "led" : result.data.screenType === "dip" ? "dip" : "lcd",
          resolution: result.data.resolution === "p6" ? "p6" : result.data.resolution === "p4" ? "p4" : result.data.resolution === "p8" ? "p8" : "p10",
          aspectRatio: result.data.aspectRatio?.toLowerCase() || "landscape",
          latitude: String(result.data.latitude || "0"),
          longitude: String(result.data.longitude || "0"),
          videoShowcase: result.data.videoShowcase || "",
          galleryImages: result.data.galleryImages?.map((img: any) => img.imagePath) || [],
          audienceTags: result.data.audienceTags || [],
        };
        setFormData(data);
      } else {
  ...
  const detectLocation = async () => {
    ...
    if (foundLat && foundLng) {
      setFormData({ ...formData, latitude: foundLat, longitude: foundLng });
      
      // Try to guess city if it's Accra central
      if (Number(foundLat) > 5.5 && Number(foundLat) < 5.7 && Number(foundLng) > -0.3 && Number(foundLng) < -0.1) {
        setFormData({ ...formData, latitude: foundLat, longitude: foundLng, city: "accra" });
      }
      return;
    }
    ...
    if (searchLower.includes("accra")) {
      setFormData({ ...formData, latitude: "5.6037", longitude: "-0.1870", city: "accra" });
    } else {
    ...
    try {
      if (type === 'gallery') {
        ...
        if (validUrls.length > 0) {
          const newGallery = [...(formData.galleryImages || []), ...validUrls];
        ...
        if (result.success && result.url) {
          if (type === 'feature') {
            setFormData({ ...formData, featureImage: result.url as string });
            toast.success("Hero image updated!");
          } else if (type === 'video') {
            setFormData({ ...formData, videoShowcase: result.url as string });
  ...
  const handleSave = async () => {
    ...
    const result = await updateBillboard(Number(id), formData);
    if (result.success) {
  ...
  if (loading || !formData) return <div className="p-20 text-center font-bold">Loading billboard data...</div>;

  return (
    <AppLayout
      title="Edit Digital Billboard"
      subtitle={`Modifying ${formData.name} (${formData.assetCode})`}
      actions={
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => router.back()} className="gap-2 h-10 px-5 border-border hover:bg-secondary/50 transition-all font-semibold">
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.category || "premium"} onValueChange={(v) => setFormData({...formData, category: v})}>
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.city || "accra"} onValueChange={(v) => setFormData({...formData, city: v})}>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input 
                        type="number" 
                        value={formData.slotDuration || 10} 
                  ...
                  <div className="bg-blue-500/5 rounded-lg p-3 border border-blue-500/10">
                    <p className="text-[11px] text-blue-500 font-medium text-center">
                      Calculation: {(formData.maxSlots || 12) * (formData.slotDuration || 10)}s total loop cycle time. Ads repeat every {(formData.maxSlots || 12) * (formData.slotDuration || 10) / 60} minutes.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-6 space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.minDuration || "1w"} onValueChange={(v) => setFormData({...formData, minDuration: v})}>
              ...
              <div className="bg-secondary/30 dark:bg-white/[0.03] rounded-xl p-4 border border-border/50">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-foreground">GH₵ {(formData.weeklyRate * 30).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-muted-foreground">Max Utilization:</span>
                    <span className="font-bold text-foreground">100% ({formData.maxSlots || 12} slots)</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {["Business", "Youth", "Premium", "Tourists"].map(tag => (
                    <Badge key={tag} variant="secondary" className="bg-accent/10 text-accent border-accent/20 hover:bg-accent/20 cursor-pointer transition-all uppercase text-[9px] font-bold">
                      {tag}
                    </Badge>
                  ))}
