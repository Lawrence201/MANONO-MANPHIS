...
type LeadStage = "new" | "contacted" | "qualified" | "negotiating" | "won" | "lost";
...
export default function LeadsBoard({ initialLeads }: { initialLeads: any[] }) {
  const [items, setItems] = useState<any[]>(initialLeads);
  const [draggedId, setDraggedId] = useState<number | null>(null);

  // Sync state if initialLeads changes (from server revalidation)
  useEffect(() => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-[10px] text-muted-foreground truncate">{lead.company || lead.serviceType}</div>
                          </div>
                        </div>
                      ...
                      <div className="text-[11px] text-muted-foreground mb-2 leading-relaxed line-clamp-2">
                        <span className="font-medium text-foreground">{lead.subject || lead.product || "General Inquiry"}</span>: {lead.message}
                      </div>
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {lead.tags.map((t: string) => (
                            <span key={t} className={cn(
                              "text-[9px] font-semibold px-1.5 py-0.5 rounded uppercase tracking-wider bg-secondary text-muted-foreground",
                            )}>{t}</span>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-border">
                        <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                          <Calendar className="w-2.5 h-2.5" /> {new Date(lead.createdAt).toLocaleDateString('en-US', {month: 'short', day: 'numeric'})}
                        </span>
                        <span className="text-[10px] font-medium text-muted-foreground">{lead.assignedTo?.split(" ")[0] || "Unassigned"}</span>
                      </div>
                    </article>
                  )
                })}
