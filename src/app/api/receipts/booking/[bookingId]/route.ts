...
async function loadImageAsBase64(imagePath: string): Promise<string | null> {
    try {
        ...
        console.log('Loading image from:', `${baseUrl}${imagePath}`);
...
async function generateReceiptPDF(data: ReceiptData): Promise<Buffer> {
    ...
    doc.text(`${data.startTime} • ${data.duration}`, col3, y);
...
export async function GET(
    ...
    try {
        ...
        // Fetch booking from database with all related data
        ...
        // Build receipt data from booking
        // Note: bh.amount in database = hall price (duration-adjusted) + add-ons total
        // We need to separate them for proper receipt display
        const facilities = booking.bookedHalls.map(bh => {
            // Calculate add-ons total for this hall
            const hallAddOnsTotal = bh.addOns.reduce((sum, addon) => sum + Number(addon.price), 0);
            // Hall-only price = stored amount - add-ons
            const hallOnlyPrice = Number(bh.amount) - hallAddOnsTotal;
            return {
                name: bh.hall.name,
                price: hallOnlyPrice
            };
        ...
        const addOns = booking.bookedHalls.flatMap(bh =>
            bh.addOns.map(addon => ({
                name: addon.name,
        ...
        // Subtotal = sum of hall prices + add-ons (which equals totalAmount)
        const facilitiesTotal = facilities.reduce((sum, f) => sum + f.price, 0);
        const addOnsTotal = addOns.reduce((sum, a) => sum + a.price, 0);
        const subtotal = facilitiesTotal + addOnsTotal;
