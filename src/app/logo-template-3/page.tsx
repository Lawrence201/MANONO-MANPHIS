...
export default function LogosTemplate3() {
  const searchParams = useSearchParams();
  const [qrUrl, setQrUrl] = useState('');

  useEffect(() => {
    // Generate the dynamic QR code URL for production/local
    setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=150x150&color=137A20&data=${encodeURIComponent(window.location.origin + '/adonis-ecofarmica')}`);
    
    // Update document title for printing
    document.title = "Logos Template 3";
            ...
            <div key={i} className={`flex flex-col items-center justify-center p-4 w-full h-[160px] text-center print:border-[0.5px] print:border-dashed print:border-gray-400 ${i >= 6 ? 'relative -top-3' : ''}`}>
              ...
              <div className="flex flex-col items-center">
                <span 
                  ...
                  style={{ 
                    backgroundImage: 'linear-gradient(to right, #137A20, #44B623, #7ED957, #137A20)',
          ...
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="relative flex flex-col w-full h-[160px] p-5 overflow-hidden bg-white print:border-[0.5px] print:border-dashed print:border-gray-400">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none">
                <img src="/ecofarm.png" alt="" className="w-36 h-36 object-contain" />
              </div>
              ...
              <div className="w-full mb-4 z-10">
                ...
                <p className={`text-[9px] font-bold tracking-[0.2em] text-[#137A20] uppercase ${montserrat.className}`}>
