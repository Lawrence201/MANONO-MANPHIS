...
export default async function AllProjectsPage() {
  ...
  const dbProjects = result.projects 
    ? result.projects.filter((p: any) => p.status === "published") 
    : [];

  const projectsToRender = dbProjects;
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 py-24 bg-[#fcfcfc]">
        {projectsToRender.length === 0 ? (
          <div className="text-center py-20">
            <h3 className="text-2xl font-bold text-gray-500">No projects published yet.</h3>
          </div>
        ) : (
            ...
            {projectsToRender.map((project: any) => (
              <div key={project.id} className="group relative rounded-md overflow-hidden bg-[#f9f9f9] transition-all duration-500 shadow-sm hover:shadow-md">
                ...
                <div className="relative h-[300px] md:h-[340px] group-hover:h-[250px] md:group-hover:h-[290px] w-full overflow-hidden transition-all duration-300 ease-in-out">
                  <Image 
                    src={project.heroImage || project.mainImage || "/construction/project_1.jpg"} 
                    alt={project.title} 
                    fill 
                ...
                <div className="p-6 md:p-8 relative bg-[#f9f9f9] group-hover:bg-[#FFD100] flex flex-col h-[180px] group-hover:h-[230px] transition-all duration-300 ease-in-out">
                  <h3 className="text-[20px] md:text-[22px] font-semibold text-[#1a1a1a] mb-4 tracking-tight line-clamp-1">{project.title}</h3>
                  ...
                  {/* View Details Link */}
                  <Link href={`/services/construction/project-details/${project.slug}`} className="inline-flex items-center gap-1.5 text-[14px] font-semibold text-gray-500 group-hover:text-[#1a1a1a] transition-colors duration-300 mt-auto">
                    View Details 
