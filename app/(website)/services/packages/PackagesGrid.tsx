...
import { FaBoxOpen, FaGem } from "react-icons/fa6"; // Changed icons
...
export default function PackagesGrid({ initialPackages }: PackagesGridProps) {
    ...
    const filteredPackages = initialPackages.filter(pkg => {
        ...
        // Currently we only have Packages, but simulating filter behavior for design
        const matchesFilter = activeFilter === 'All' || activeFilter === 'Standard';
        ...
        <>
            ...
            <div className={styles.searchSection}>
                ...
                <div className={styles.filterWrapper}>
                    ...
                    <div className={styles.filterTabs}>
                        ...
                        <button
                            className={`${styles.filterTab} ${activeFilter === 'Standard' ? styles.active : ''}`}
                            onClick={() => setActiveFilter('Standard')}
                        >
                            <FaBoxOpen size={14} /> Standard
                        </button>
                        <button
                            className={`${styles.filterTab} ${activeFilter === 'Premium' ? styles.active : ''}`}
                            onClick={() => setActiveFilter('Premium')}
                        >
                            <FaGem size={14} /> Premium
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className={styles.grid}>
                {filteredPackages.map((pkg) => {
                    ...
                    // Check if price is numeric or text like "1000", if it's "Free" or "Custom" handle gracefully
                    // For now assuming numeric in string
                    const formattedPrice = priceString; // Keep as string for currency symbol concatenation
                    const currencyPrice = `₵${formattedPrice}`;

                    return (
                        <div key={pkg.id} className={styles.card}>
                            <div className={styles.imageWrapper}>
                                <span className={styles.tag}>Event Package</span>
                                <Image
                ...
                {filteredPackages.length === 0 && (
                    <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '60px', color: '#666' }}>
                        <p>{activeFilter === 'Premium' ? 'No premium packages currently listed.' : 'No packages found matching your search.'}</p>
                    </div>
                )}
            </div>
        </>
