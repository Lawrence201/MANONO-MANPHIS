...
async function getGroupRetreats(): Promise<PackageData[]> {
    try {
        ...
        console.error("Failed to fetch group retreats:", error);
        return [];
    }
}

export default async function GroupRetreatsPage() {
    const packages = await getGroupRetreats();
    console.log(`[GroupRetreatsPage] Fetched ${packages.length} group retreats`);
        ...
        <>
            ...
            <div style={{ padding: '32px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                    <div>
                        <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#111827' }}>Group Retreats</h1>
                        <p style={{ color: '#6B7280', marginTop: '4px' }}>Manage your group retreat packages.</p>
                    </div>
                    <Link
                        href="/admin/add-group-retreats"
                        style={{
                    ...
                    >
                        <Plus size={20} /> Add Group Retreat
                    </Link>
                </div>
