...
async function getEventPackages(): Promise<PackageData[]> {
    try {
        // Professional Prisma Client approach - ONLY fetch 'event' type packages
...
export default async function PackagesPage() {
    ...
    console.log(`[PackagesPage] Fetched ${packages.length} event packages`);

    return (
        <div style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                <div>
                    <h1 style={{ fontSize: '24px', fontWeight: 700, color: '#111827' }}>Event Packages</h1>
                    <p style={{ color: '#6B7280', marginTop: '4px' }}>Manage your event packages.</p>
                </div>
                <Link
                ...
                >
                    <Plus size={20} /> Add Event Package
                </Link>
            </div>
